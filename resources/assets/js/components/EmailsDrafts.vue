<template>
    <div class="emails-drafts" v-if="successfulFetch" v-cloak>
        ...
        <table class="table">
            ...
            <tbody>
                <tr v-for="email in orderedEmails">
                    ...
                    <td><router-link v-bind:to="{ name: 'emails.edit', params: { id: email.id } }" class="btn btn-default btn-xs"><i class="fa fa-pencil-square-o"></i></router-link></td>
                    <td></td>
        ...
        methods: {
            fetchEmails(orderAttr, orderToggle) {
                ...
                vm.$http.get(vm.resourceUrl, {params : params}).then(function(response) {
                    ...
                    if ( emails && emails.data && emails.data.length ) {
                        ...
                        var message = vm.searching ? 'Your search returned no results. Please try again with different keywords' : 'You don\'t have any email drafts.';
            ...
            quickEditEmails() {
                ...
                if ( action.length && selected.length ) {
                    ...
                    vm.$http.put(vm.resourceUrl + '/' + action + '/quick-edit', {emails: selected, draft: 1}).then(function (response) {
                        if (response.data && response.data.success) {
                            progress.finish();
                            vm.quickEdit = '';
                            swal({
