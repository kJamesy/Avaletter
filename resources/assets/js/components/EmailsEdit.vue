<template>
    <div class="emails-edit" v-if="successfulFetch" v-cloak>
        <div class="clearfix">
        ...
        <div style="margin-top: 50px">
            <button class="btn-danger btn btn-xs" v-on:click.prevent="deleteEmail" style="float:right" title="Delete"><i class="fa fa-times-circle-o"></i></button>
        </div>
    </div>
</template>
    ...
    export default{
        ...
        methods: {
                ...
                vm.$http.get(vm.resourceUrl + '/' + vm.id).then(function(response) {
                    if (response.data) {
                        if ( response.data.email ) {
                            vm.email.email_edition_id = response.data.email.email_edition_id;
                            vm.email.subject = response.data.email.subject;
                            vm.email.body = response.data.email.body;
                            ...
                            progress.fail();
                            swal({ title: "An Error Occurred", text: 'The email draft does not exist', type: 'error', animation: 'slide-from-top'}, function() {
                                vm.$router.replace({ name: 'emails.drafts' });
                            });
                        }
                    }
                }, function(error) {
            ...
            updateEmail() {
                ...
                vm.$http.put(vm.resourceUrl + '/' + vm.id, vm.email).then(function(response) {
                    progress.finish();

                    if ( response.data && response.data.queued ) {
                        swal({ title: "Success", text: response.data.queued, type: 'success', animation: 'slide-from-bottom'}, function() {
                            vm.$router.replace({ name: 'emails.index' });
                        });
                    }
                    else if ( response.data && response.data.saved )
                        swal({ title: "Success", text: response.data.saved, type: 'success', animation: 'slide-from-bottom', timer: 3000 });
            ...
            deleteEmail() {
                ...
                swal({
                    title: "Delete email draft: " + email.subject + "?",
                    text: "The email draft will be permanently deleted",
                    type: "warning",
                    ...
                    vm.$http.delete(vm.resourceUrl + '/' + vm.id, { params : { destroy: 1 } } ).then(function(response) {
